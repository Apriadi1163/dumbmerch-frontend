...
import Apriadi from "../assets/Apriadi.png"
    ...
    <>
      {contact ? (
        <>
              ...
              <div key={index}>
                <div className={`d-flex py-1 ${item.idSender === user.id ? "justify-content-end": "justify-content-start"}`}>
                  {item.idSender !== user.id && (
                    <img src={contact.profile?.image || Apriadi} className="rounded-circle me-2 img-chat" alt="bubble avatar" />
                  )}
                  <div
          ...
          <div style={{ height: '6vh' }}className="px-2 d-grid">
            <input 
